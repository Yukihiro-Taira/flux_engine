...
fn draw_endpoint(
    ...
    egui::Rect::from_center_size(
        endpoint.position,
        egui::vec2(hit_size, hit_size),
    );

    let response = ui.interact(
        hit_rect,
        egui::Id::new(endpoint.id),
        egui::Sense::click(),
    );
...
pub fn show(
    ...
    .show(context, |ui| {
        ...
        let x_direction = project_axis(
            cgmath::Vector3::unit_x(),
            camera_right,
            camera_screen_up,
        );

        let y_direction = project_axis(
            cgmath::Vector3::unit_y(),
            camera_right,
            camera_screen_up,
        );

        let z_direction = project_axis(
